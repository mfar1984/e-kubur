        ...
        <div class="container mx-auto px-0 py-0">
            ...
            <div class="bg-white shadow-lg border-x border-gray-200 p-6">
                ...
                <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 mb-1">Kumpulan Akses</h1>
                        <p class="text-xs text-gray-600">Senarai kumpulan akses yang berdaftar dalam sistem</p>
                    </div>
                    <div class="flex items-center justify-center md:justify-end">
                        <a href="{{ route('roles.create') }}" class="inline-flex items-center justify-center px-3 py-2 bg-blue-600 text-white text-xs rounded-xs hover:bg-blue-700">
                            <span class="material-icons text-[10px] mr-2">add</span>
                            Tambah Kumpulan
                        </a>
                    </div>
                </div>
                ...
                <!-- Filters & Search -->
                <form method="GET" action="{{ route('roles.index') }}" class="mb-4 space-y-3 md:space-y-0 md:flex md:space-x-3">
                    <div class="flex-1">
                        <div class="relative">
                            <span class="material-icons absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">search</span>
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama kumpulan..." class="w-full pl-9 pr-3 py-2 border border-gray-200 rounded-xs text-xs focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder-gray-500 text-gray-900" />
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-2 md:flex md:space-x-2">
                        ...
                        <button type="button" onclick="window.location.href='{{ route('roles.index') }}'" class="px-4 py-2 bg-red-100 text-red-700 text-xs rounded-xs hover:bg-red-200 flex items-center justify-center">
                ...
                <div class="hidden md:block overflow-x-auto bg-gray-50 rounded-xs border border-gray-200">
                    <table class="min-w-full text-left text-sm">
                        <thead class="bg-blue-100 text-gray-600">
                            <tr>
                                <th class="px-4 py-2 font-medium text-xs">Nama Kumpulan</th>
                                <th class="px-4 py-2 font-medium text-xs">Izin</th>
                                <th class="px-4 py-2 font-medium text-xs">Bilangan Pengguna</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-white">
                                <td class="px-4 py-1">
                                    <div class="font-medium text-gray-900 text-xs">{{ $role->name }}</div>
                                </td>
                                <td class="px-4 py-2 text-gray-900 text-xs">
                                    ...
                                    @else
                                        <span class="text-gray-500">Tiada izin</span>
                                    @endif
                                </td>
                                <td class="px-4 py-2 text-gray-600 text-xs">{{ $role->users()->count() }} pengguna</td>
                                <td class="px-4 py-2 text-gray-600 text-xs">{{ $role->created_at->format('d/m/Y H:i') }}</td>
                                <td class="px-4 py-1 text-center space-x-1">
                                    <a href="{{ route('roles.show', $role) }}" class="text-gray-700 hover:text-gray-600 action-icon" title="Lihat" aria-label="Lihat">
                                        <span class="material-icons text-[8px]">visibility</span>
                                    </a>
                                    <a href="{{ route('roles.edit', $role) }}" class="text-blue-600 hover:text-blue-800 action-icon" title="Edit" aria-label="Edit">
                                        <span class="material-icons text-[8px]">edit</span>
                                    </a>
                                    <button type="button" onclick="showDeleteModal('{{ $role->id }}', '{{ $role->name }}')" class="text-red-600 hover:text-red-600 action-icon" title="Padam" aria-label="Padam">
                ...
                <div class="md:hidden space-y-3">
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                        ...
                        <div class="flex items-center justify-between mb-3">
                            <div class="flex-1">
                                <h3 class="font-semibold text-gray-900 text-sm">{{ $role->name }}</h3>
                                <p class="text-xs text-gray-500">{{ $role->created_at->format('d/m/Y H:i') }}</p>
                            </div>
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('roles.show', $role) }}" class="p-2 text-gray-600 hover:text-gray-800 rounded-full hover:bg-gray-100" title="Lihat">
                                    <span class="material-icons text-sm">visibility</span>
                                </a>
                                <a href="{{ route('roles.edit', $role) }}" class="p-2 text-blue-600 hover:text-blue-800 rounded-full hover:bg-blue-50" title="Edit">
                                    <span class="material-icons text-sm">edit</span>
                                </a>
                                <button type="button" onclick="showDeleteModal('{{ $role->id }}', '{{ $role->name }}')" class="p-2 text-red-600 hover:text-red-800 rounded-full hover:bg-red-50" title="Padam">
                        ...
                        <div class="grid grid-cols-2 gap-3 text-xs">
                            ...
                            <div>
                                ...
                                <p class="text-gray-900 font-medium">{{ $role->users()->count() }} pengguna</p>
                            </div>
                        </div>
                    </div>
                    @empty
                    <div class="bg-white border border-gray-200 rounded-lg p-8 text-center">
                        ...
                        <p class="text-gray-500 text-sm">Tiada data kumpulan akses dijumpai.</p>
                    </div>
                    @endforelse
                </div>
                ...
                <div class="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                    <div class="text-sm text-gray-700 text-center sm:text-left">
                        Menunjukkan {{ $roles->firstItem() }} hingga {{ $roles->lastItem() }} daripada {{ $roles->total() }} kumpulan
                    </div>
