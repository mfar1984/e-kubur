    ...
    <main class="flex-1">
        <div class="container mx-auto px-0 py-0">
            <div class="bg-white shadow-lg border-x border-gray-200 p-6">
                ...
                <div class="mb-6">
                    <div>
                        <h1 class="text-xl font-bold text-gray-900 mb-1">Edit Ahli PPJUB</h1>
                        <p class="text-xs text-gray-600">Kemaskini maklumat ahli PPJUB</p>
                    </div>
                </div>

                <!-- Form -->
                <form method="POST" action="{{ route('ppjub.update', $ppjub) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-4 md:gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="text" id="nama" name="nama" value="{{ old('nama', $ppjub->nama) }}" required
                        ...
                        <div>
                            ...
                            <input type="text" id="no_ic" name="no_ic" value="{{ old('no_ic', $ppjub->no_ic) }}" required
                        ...
                        <div>
                            ...
                            <input type="text" id="telefon" name="telefon" value="{{ old('telefon', $ppjub->telefon) }}" required
                        ...
                        <div>
                            ...
                            <input type="email" id="email" name="email" value="{{ old('email', $ppjub->email) }}" required
                        ...
                        <div>
                            ...
                            <select id="status" name="status" required
                                ...
                                <option value="Aktif" {{ old('status', $ppjub->status) == 'Aktif' ? 'selected' : '' }}>Aktif</option>
                                <option value="Tidak Aktif" {{ old('status', $ppjub->status) == 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="date" id="tarikh_keahlian" name="tarikh_keahlian" value="{{ old('tarikh_keahlian', $ppjub->tarikh_keahlian ? $ppjub->tarikh_keahlian->format('Y-m-d') : '') }}" required
                    ...
                    <div>
                        ...
                        <textarea id="alamat" name="alamat" rows="3" required
                            ...
                            placeholder="Masukkan alamat lengkap">{{ old('alamat', $ppjub->alamat) }}</textarea>
                        @error('alamat')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
